...
const PURPOSE_CONFIG = {
  ...
  phone_verify:   { channel: 'sms',   kind: 'otp',   ttlMin: env.otp.ttlMinutes },
  password_reset: { channel: 'email', kind: 'token', ttlMin: env.passwordReset.ttlMinutes },
};
...
export const verificationService = {
  async issue({ account, purpose, sendTo }) {
    ...
    await VerificationToken.deleteMany({
      account: account._id, purpose, consumedAt: null,
    });

    const rawCode = cfg.kind === 'otp' ? generateOtp(env.otp.length) : generateToken(32);
    ...
    // NEVER return the raw code in prod responses.
    return { sendTo, expiresInMin: cfg.ttlMin };
  },
  ...
  async consume({ account, purpose, code }) {
    ...
    if (doc.expiresAt < new Date()) throw new AppError('Code expired', 410, 'EXPIRED');
